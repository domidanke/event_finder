                  ...
                  child: Opacity(
                    ...
                    child: CustomButton(
                        onPressed: () async {
                          ...
                          await EventDocService().rateEvent(event, eventRating);
                          if (event.artists.isNotEmpty &&
                              artistRatings.isNotEmpty) {
                            await UserDocService().rateArtists(artistRatings);
                            moreThanOneRating = true;
                          }
                          await UserDocService().finishRatingForUser(event.uid);
                          if (mounted) {
                            ScaffoldMessenger.of(context).showSnackBar(
                                PopupService().getCustomSnackBar(
                                    'Bewertung${moreThanOneRating ? 'en' : ''} abgeschickt'));
                            Navigator.pop(context);
                          }
                        },
